...
出题、答案评价或首次生成报告时，如果模型服务失败，接口返回 502，并在独立事务中把对应面试标记为 `failed`。`failed` 是终态，后续出题、答题和首次报告生成请求返回 409，不自动重试；创建面试时的岗位分析发生在会话入库前，因此该阶段失败不会产生可标记的会话。已保存的报告优先从数据库读取，即使模型服务暂时不可用，历史报告仍可查看。
